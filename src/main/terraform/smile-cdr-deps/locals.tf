locals {
  ...
  # Extra Secrets
  cdr_extra_secrets = [
    for secret in var.extra_secrets: {
      name = try(secret.name,false)
      name_override = try(secret.name_override,false)
      existing_arn = try(secret.arn,null)
      existing_kms_arn = try(secret.kms_arn,null)
    }
  ]
  ...
  secrets_enabled = var.enable_cdr_regcred_secret || var.enable_cdr_license_secret || local.enable_cdr_rds_secrets || (length(var.extra_secrets) > 0)  ? true:false

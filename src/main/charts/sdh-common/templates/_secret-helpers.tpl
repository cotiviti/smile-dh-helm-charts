...
{{- define "sdhCommon.imagePullSecrets" -}}
  ...
  {{- range $i, $v := .Values.image.imagePullSecrets -}}
    {{- /* This is the autogenerated secret name used when using SSCSI */ -}}
    {{- $secretName := printf "%s-%s-image-pull-secrets" $.Release.Name $chartShortName -}}
...
{{- define "sdhCommon.extraSecrets" -}}
  ...
  {{- range $k, $v := (.Values.secrets) -}}
    {{- $secretName := $k -}}
    {{- if eq $v.type "sscsi" -}}
    {{- $secretFullName := printf "%s-%s-%s" $.Release.Name $chartShortName $secretName -}}
...
{{- define "sdhCommon.sscsi.secretProviderClassName" -}}
  {{- $chartShortName := include "chart.shortname" . -}}
  {{- printf "%s-%s" $chartShortName .Release.Name -}}
{{- end -}}
...
{{ define "sdhCommon.secrets.k8sSecretObjects" }}
  ...
  {{- range $secret := $allSecrets -}}
    {{- if and (eq $secret.type "k8sSecret") $secret.autoCreate -}}
      {{- $existingSecretObj := (lookup "v1" "Secret" $.Release.Namespace $secret.name) | default dict -}}

...
![Version: 1.0.0-pre.28](https://img.shields.io/badge/Version-1.0.0--pre.28-informational?style=flat-square) ![SmileCDRVersion: 2022.11.R01](https://img.shields.io/badge/Smile%20CDR%20Version-2022.11.R01-informational?style=flat-square)
...
* Horizontal Auto-Acaling (Within bounds of Smile CDR licence) - to ensure cost effective use of compute resources
* Coming soon...
...
* Access to an OCR container repository with the required Smile CDR Docker images
...
* AWS IAM Role for the SmileCDR application.
...
it to deploy SmileCDR in a way that works in your environment.
...
### Configure Resources
As SmileCDR is a high performance Java based application, special consideration needs
...
database and will do a new install. In addition to this, if you were to configure multiple replicas,
...
* A secret containing the connection credentials.
  * It is common practice to include all connection credentials in DB secrets, this way it becomes simple
  to manage the database without having to reconfigure SmileCDR. e.g. when 'restoring' an RDS instance, the
  DB cluster name will typically change. If this is kept inside the secret (As it is with RDS when using AWS
  Secrets Manager) then any such change will be automatically applied.
  * The secret can be a plain Kubernetes secret that you provision externally, or it can be a secret in a
  secure secrets vault. This is the preferred option. At this time, only AWS Secrets Manager is supported.

If using AWS Secrets Manager, set the `credentialsSource` to `sscsi-aws`. If you have created a `Secret` object
in Kubernetes, set it to `k8s`.

Configure your secret as per the following yaml fragment, which will get all
credential details from the secret. If the details fields are different in the provided secret, they can be
...
database:
  external:
    ...
    - secretName: smilecdr
      ...
      passKey: password
```
If the secret does not contain certain fields, they can be overriden like so:
#### `my-values.yaml`
```yaml
- secretName: smilecdr
  module: clustermgr
...
The `module` key is important here as it tells the Helm Chart logic which module uses this database.
If there is only one database (for external DBs) or user(for CrunchyData provisioned DB) listed here,
then this database will be used for all modules.

If you provide multiple databases/users, then the database will be used by the SmileCDR module that is
referenced by the `module` key for that database/user.
...
fragment to automate provisioning of a Kafka cluster. Your SmileCDR instance will then be automatically
